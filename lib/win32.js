...
var write = function (log, src, type, msg, id, callback) {
  ...
  log = log || 'APPLICATION'
  log = EVENT_TYPES.indexOf(log.toUpperCase()) >= 0 ? log : 'APPLICATION'
  type = (type || 'INFORMATION').trim().toUpperCase()
  type = (MESSAGE_TYPES.indexOf(type.trim().toUpperCase()) >= 0 ? type : 'INFORMATION').trim().toUpperCase()
...
var EventLogger = function (config) {
  ...
  Object.defineProperties(this, {
    ...
    eventLog: {
      ...
      set: function (value) {
        if (value) {
          this._logname = EVENT_TYPES.indexOf(value.toUpperCase()) >= 0 ? value.toUpperCase() : 'APPLICATION'
        }
      }
    },
